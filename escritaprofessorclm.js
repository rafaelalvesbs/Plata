window.Router.register('escritaprofessorclm', async () => {
   ...
   const { 
    ...
    let propostasCache = [];
    let idEditando = null; // Controle de edição
    ...
    window.handleImagePreview = (input) => {
        if (input.files && input.files[0]) {
            ...
            if (file.size > 2 * 1024 * 1024) {
                window.mostrarAvisoErro("A imagem é muito pesada! Escolha uma de até 2MB.");
                input.value = '';
                return;
            }
    ...
    try {
        const dados = {
            ...
            professorId: window.auth?.currentUser?.uid || "PROF_ID"
        };
        ...
        window.mostrarToastSucesso("Publicado com sucesso!");
        ...
        // Atualiza a lista e volta para a aba de listagem
    ...
    try {
        const q = query(collection(db, "atividades_enviadas"), where("tipo", "==", "escrita"));
...
window.removerFotoEscrita = () => {
        ...
        try {
            ...
            snap.forEach(d => {
                ...
                if (data.semestre && data.senha) {
                    // O Map garante que nomes repetidos sejam sobrescritos, mantendo apenas um
                    mapaUnico.set(data.semestre, data.senha);
                }
            });
            ...
            turmasLimpas.sort((a, b) => a.semestre.localeCompare(b.semestre, undefined, { numeric: true }));

            div.innerHTML = '';
            turmasLimpas.forEach(t => {
                const btn = document.createElement('button');
                btn.className = 'btn-turma-pill';
                btn.innerText = t.semestre;
    ...
    try {
        const q = query(collection(db, "redacoes"), where("status", "==", "pendente"));
        const snap = await getDocs(q);
        ...
        if (chavesTurmas.length === 0) {
            grid.innerHTML = '<p style="padding:20px; color:#64748b;">Nenhuma redação aguardando correção.</p>';
            return;
        }
        ...
        grid.innerHTML = '<p style="color:red;">Erro ao processar lista de turmas.</p>';
    }
};
...
window.enviarFeedbackRedacao = async (redacaoId) => {
    ...
    if (!comentario.trim()) {window.mostrarAvisoErro("Escreva um comentário antes de enviar."); return;}
    ...
    try {
        ...
        if(document.getElementById('modal-redacao-texto')) document.getElementById('modal-redacao-texto').style.display = 'none';
        window.carregarTodasRedacoesRecebidas(); // Atualiza a lista
    } catch (e) {
...
window.carregarRedacoesCorrigidas = async () => {
    ...
    container.innerHTML = '<p style="padding:20px; color:#64748b;">Buscando corrigidas...</p>';
    try {
        const q = query(collection(db, "redacoes"), where("status", "==", "corrigida"), orderBy("dataCorrecao", "desc"));
        const snap = await getDocs(q);
        corrigidasCache = [];
        snap.forEach(d => corrigidasCache.push({ id: d.id, ...d.data() }));
        paginaAtualCorrigidas = 1;
        window.renderizarListaCorrigidas();
    } catch (e) { 
        console.error(e);
        container.innerHTML = '<p style="color:red;">Erro ao carregar corrigidas.</p>'; 
    }
};
            ...
            <div class="card-premium" style="padding: 25px;">
                ...
                <div style="display: flex; gap: 15px; width: 100%;">
                    <div style="flex: 2;">
                        ...
                        <input type="text" id="titulo-escrita-prof" class="input-premium" placeholder="Ex: Redação sobre Minha rotina" maxlength="100">
                    </div>
                    <div style="flex: 1;">
