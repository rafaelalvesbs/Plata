window.Router.register('calendario', async () => {
    ...
    const getStorageKey = () => 'eventos_calendario';
        ...
        const modalAddHTML = `
          <div id="modal-agendar-cal" style="position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,48,88,0.4); display:flex; justify-content:center; align-items:center; z-index:10000; padding:20px; backdrop-filter: blur(6px); animation: fadeInCal 0.2s ease;">
            <div style="background:white; padding:30px; border-radius:20px; max-width:400px; width:100%; box-shadow:0 20px 40px rgba(0,0,0,0.2); transform: scale(0.9); animation: scaleUpCal 0.2s forwards;">
              ...
              <h3 style="color:${azulPadrao}; margin:0 0 10px 0; font-size:1.2rem; font-weight:800; text-align:center;">${idEdicao ? 'Editar Evento' : 'Novo Evento'}</h3>
              <p style="color:#64748b; font-size:0.9rem; text-align:center; margin-bottom:20px;">Para o dia: ${data.split('-').reverse().join('/')}</p>
              
              <input type="text" id="input-titulo-cal" value="${tituloAtual}" placeholder="Ex: Aula de conversação..." style="width:100%; padding:14px; border-radius:12px; border:2px solid #e2e8f0; margin-bottom:25px; outline:none; font-family:inherit; font-size:1rem; box-sizing:border-box;">
    ...
    const renderizarListaEventos = () => {
        ...
        const eventos = JSON.parse(localStorage.getItem(getStorageKey()) || '[]');
        const hoje = new Date().setHours(0,0,0,0);
    ...
    setTimeout(atualizarCalendarioCompleto, 100);
    ...
    <div style="display: grid; grid-template-columns: 1fr 350px; gap: 20px; width: 100%;">
        <div class="card" style="box-shadow: 0 10px 25px rgba(0,0,0,0.05); border-radius: 20px; background: white; padding: 25px; border: 1px solid #f1f5f9;">
            ...
            <table style="width: 100%; border-collapse: collapse; text-align: center;">
                <thead>
                    <tr style="color: #94a3b8; font-size: 0.75rem; font-weight: 800; text-transform: uppercase;">
        ...
        <div class="card" style="box-shadow: 0 10px 25px rgba(0,0,0,0.05); border-radius: 20px; background: white; padding: 25px; display: flex; flex-direction: column; border: 1px solid #f1f5f9;">
            ...
            <div id="lista-eventos-dinamica" style="flex: 1; min-height:250px;"></div>
