window.Router.register('mensagensclm', async () => {
    ...
    window.abrirMensagemPorId = async (id) => {
        ...
        const interlocutor = msg.remetenteId === user.uid ? msg.destinatarioNome : (msg.remetenteTipo === 'professor' ? 'Prof. ' + msg.remetenteNome : msg.remetenteNome || msg.remetente);
                ...
                <div class="footer-expandida">
                    <div style="display:flex; gap:10px; width:100%; justify-content: space-between;">
                        ...
                        <button id="btn-abrir-resposta" 
                            ...
                            style="display: ${msg.remetenteId !== user.uid ? 'block' : 'none'}; background:${azulPadrao}; color:white; border:none; padding:12px; flex:1; border-radius:12px; font-weight:800; cursor:pointer; font-size:0.7rem; text-transform:uppercase;">
                            Responder
                        </button>
                    </div>
                </div>
    ...
    // FUNÇÃO DE RESPOSTA CORRIGIDA (Busca dados no cache pelo ID)
    window.enviarRespostaExpandida = async (idOriginal) => {
        ...
        if(!texto) return window.mostrarFeedback("Digite uma mensagem antes de enviar", "erro");
        if(!msgOriginal) return window.mostrarFeedback("Erro ao localizar mensagem original", "erro");

        try {
            const user = auth.currentUser;
            // O destinatário da resposta é sempre quem enviou a original
            const destId = msgOriginal.remetenteId;
            const destNome = msgOriginal.remetenteTipo === 'professor' ? msgOriginal.remetenteNome : msgOriginal.remetente;
            const assuntoOriginal = msgOriginal.titulo || "Sem Assunto";

            await addDoc(collection(db, "mensagens_diretas"), {
                titulo: assuntoOriginal.startsWith("Re:") ? assuntoOriginal : "Re: " + assuntoOriginal,
                conteudo: texto,
                remetente: user.displayName || "Aluno",
                remetenteId: user.uid,
                destinatarioId: destId,
                destinatarioNome: destNome.replace(/^Prof\.\s+|Professor\s+/i, ''),
                dataEnvio: new Date(),
                tipo: "direta_privada",
                lida: false
            });

            window.mostrarFeedback("Sua resposta foi enviada!");
            window.carregarChatAluno(); // Recarrega tudo para atualizar listas
        } catch (e) { 
            console.error(e);
            window.mostrarFeedback("Erro ao enviar resposta", "erro"); 
        }
    };
            ...
            <div id="modal-confirm-exclusao" style="position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(15, 23, 42, 0.7); backdrop-filter:blur(8px); z-index:20000; display:flex; justify-content:center; align-items:center; animation: fadeIn 0.2s forwards;">
                <div style="background:white; padding:35px; border-radius:30px; width:90%; max-width:400px; text-align:center; box-shadow:0 25px 50px -12px rgba(0,0,0,0.25);">
                    ...
                    <div style="display:flex; gap:12px;">
                        <button onclick="document.getElementById('modal-confirm-exclusao').remove()" style="flex:1; padding:14px; border:none; border-radius:15px; font-weight:800; cursor:pointer; background:#f1f5f9; color:#64748b; font-size:0.75rem;">CANCELAR</button>
                        <button onclick="window.executarExclusao('${id}')" style="flex:1; padding:14px; border:none; border-radius:15px; font-weight:800; cursor:pointer; background:#dc2626; color:white; font-size:0.75rem;">EXCLUIR</button>
                    </div>
                </div>
            </div>`;
    ...
    window.enviarNovaMensagem = async () => {
        ...
        try {
            const user = auth.currentUser;
            await addDoc(collection(db, "mensagens_diretas"), {
                titulo: ass, conteudo: cont, remetente: user.displayName || "Aluno", remetenteId: user.uid,
                destinatarioId: destId, destinatarioNome: destNome, dataEnvio: new Date(), tipo: "direta_privada", lida: false
            });
            document.getElementById('modal-novo').remove();
            window.mostrarFeedback("Enviada!");
            window.carregarChatAluno();
        } catch (e) { window.mostrarFeedback("Erro", "erro"); }
    };
